...
class DatasetBuilder:
    ...
    def process_file(self, audio_dir: str, filename: str, max_duration_ms: int = 30000):
        """Process a file by splitting it into segments.

        Args:
            audio_dir (str): the directory containing the audio files
            filename (str): the name of the file
            max_duration_ms (int, optional): the max duration of the audio. Defaults to 30000.
        """
        ...
        for i in range(0, total_duration, max_duration_ms):
            ...
            output_path = os.path.join(audio_dir, segment_filename)
            segment.export(output_path, format="mp3")

        # Remove the original file
        os.remove(audio_path)

    def process_audio_files(self, audio_dir: str, max_duration_ms: int = 30000):
        """
        Process audio files by splitting them into segments.

        Args:
            input_dir (str): the directory containing the audio files
            max_duration_ms (int, optional): the maximal duration in ms of the audio. Defaults to 30000.
        """
        ...
        with ThreadPoolExecutor() as executor:
            futures = [
                executor.submit(self.process_file, audio_dir, filename, max_duration_ms)
    ...
    def build_ds(self, audio_dir: str, dataset_dir: str, split_audio: bool = False):
        ...
        if split_audio:
            self.process_audio_files(audio_dir)
